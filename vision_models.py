...
class DepthEstimationModel(BaseModel):
    ...
    def __init__(self, gpu_number=0, model_type='DPT_Large'):
        ...
        with HiddenPrints('DepthEstimation'):
            ...
            depth_estimation_model = hub.load('MiDaS/', model_type, pretrained=False, source='local').to('cpu')
            depth_estimation_model.load_state_dict(torch.load('pretrained_models/depth/dpt_large_384.pt', map_location='cpu'))
            depth_estimation_model = depth_estimation_model.to(self.dev)
...
class CLIPModel(BaseModel):
    ...
    def __init__(self, gpu_number=0, version="ViT-L/14@336px"):  # @336px
        ...
        with HiddenPrints('CLIP'):
            model, preprocess = clip.load(config.clip_version, device=self.dev)
            model.eval()
            model.requires_grad_ = False
        self.model = model

        self.transform = self.get_clip_transforms_from_tensor(336 if "336" in config.clip_version else 224)

        with HiddenPrints('CLIP2'):
            model, preprocess = clip.load('RN50x16', device=self.dev)
            model.eval()
            model.requires_grad_ = False
        self.model2 = model
        self.transform2 = self.get_clip_transforms_from_tensor(384)
        self.models = [self.model, self.model2]
        self.transforms = [self.transform, self.transform2]
        self.negative_text_features = [self.clip_negatives("photo of ", model_index=0), self.clip_negatives("photo of ", model_index=1)]
    ...
    @torch.no_grad()
    def binary_score(self, image: torch.Tensor, prompt, negative_categories=None, model_index=0):
        is_video = isinstance(image, torch.Tensor) and image.ndim == 4
        if is_video:  # video
            image = torch.stack([self.transforms[model_index](image[i]) for i in range(image.shape[0])], dim=0)
        else:
            image = self.transforms[model_index](image).unsqueeze(0).to(self.dev)
        ...
        if negative_categories is None:
            if self.negative_text_features is None:
                self.negative_text_features[model_index] = self.clip_negatives(prompt_prefix, model_index=model_index)
            negative_text_features = self.negative_text_features[model_index]
        else:
            negative_text_features = self.clip_negatives(prompt_prefix, negative_categories, model_index)

        text = self.clip.tokenize([prompt]).to(self.dev)

        image_features = self.models[model_index].encode_image(image.to(self.dev))
        image_features = F.normalize(image_features, dim=-1)

        pos_text_features = self.models[model_index].encode_text(text)
    ...
    @torch.no_grad()
    def clip_negatives(self, prompt_prefix, negative_categories=None, model_index=0):
        if negative_categories is None:
        ...
        negative_text_features = self.models[model_index].encode_text(negative_tokens)
    ...
    @torch.no_grad()
    def compare(self, images: list[torch.Tensor], prompt, return_scores=False, model_index=0, logit_scale=False):
        images = [self.transforms[model_index](im).unsqueeze(0).to(self.dev) for im in images]
        ...
        image_features = self.models[model_index].encode_image(images.to(self.dev))
        image_features = F.normalize(image_features, dim=-1)

        text_features = self.models[model_index].encode_text(text)
        text_features = F.normalize(text_features, dim=-1)
        if logit_scale:
            sim = (self.models[model_index].logit_scale.exp() * image_features @ text_features.T).squeeze(dim=-1)
        else:
    ...
    def forward(self, image, prompt, task='score', return_index=True, negative_categories=None, return_scores=False, model_index=0, logit_scale=False):
        if task == 'classify':
            categories = prompt
            clip_sim = self.classify(image, categories, return_index=return_index)
            out = clip_sim
        elif task == 'score':
            clip_score = self.binary_score(image, prompt, negative_categories=negative_categories, model_index=model_index)
            out = clip_score
        else:  # task == 'compare'
            idx = self.compare(image, prompt, return_scores, model_index, logit_scale)
            out = idx
...
class MaskRCNNModel(BaseModel):
    ...
    @torch.no_grad()
    def detect(self, images: torch.Tensor, return_labels=False, return_all=True):
        if type(images) != list:
            images = [images]
        ...
        for i in range(len(images)):
            ...
            if return_all:
                pass
            elif return_labels:  # In the current implementation, we only return labels
    ...
    def forward(self, image, return_labels=False, return_all=True):
        obj_detections = self.detect(image, return_labels, return_all)
        # Move to CPU before sharing. Alternatively we can try cloning tensors in CUDA, but may not work
        if return_all:
            assert len(obj_detections) == 1
            boxes = torch.stack([obj_detections[0]['boxes'][:, 0], image.shape[1] - obj_detections[0]['boxes'][:, 3], obj_detections[0]['boxes'][:, 2], image.shape[1] - obj_detections[0]['boxes'][:, 1]], dim=1)
            return {'labels': [self.categories[d] for d in obj_detections[0]['labels']], 'boxes': boxes.tolist()}
...
class CodexModel(BaseModel):
    ...
    def forward(self, prompt=None, extended_prompt=None):

        result = self.forward_(extended_prompt)
        if not isinstance(prompt, list) and isinstance(result, list):
            result = result[0]
